(function() {
	...
	var app = new Vue({
		...
		methods: {
			turnTo:function(name,id,typeId){//id是下面113行跳转用的，避免重复创建
				console.log(id)
				mui.openWindow({
				    url:name+'.html?type='+this.typetype+'&&key='+this.keykey+'&&typeId='+typeId,
				    id:id+'.html',
				    styles:{
				    	popGesture: 'close',
				    },
			...
			getUrlObj:function () { //获取url？号后面的值，以对象的形式存在
				...
				this.typetype = Obj.type;
				this.keykey = Obj.key;
				this.typeName = Obj.typeName;
				this.jumpid = Obj.jumpid
...
//				plus.nativeUI.showWaiting();
				NetUtil.ajax("/Identity/Friendslist",{
					type:this.typetype,
					key:this.keykey,
